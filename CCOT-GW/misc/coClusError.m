function [err,err_r,err_c]=coClusError(z,w,hatz,hatw)

%% Compute the Co-clustering error 
%% The function also reogornize hatz and hatw to get as close as possible to z and w
%% Input: - z and w (true partitions of rows and columns)
%%        - hatz and hatw (estimated partitions with a given algorithm)
%% Output: co-clustering error, error on rows, errow on columns
...
confmat1=crosstab(z,hatz);
confmat2=crosstab(w,hatw);

err_r=(n-trace(confmat1))/n;err_c=(d-trace(confmat2))/d;
err=err_r+err_c-err_r*err_c;
end
