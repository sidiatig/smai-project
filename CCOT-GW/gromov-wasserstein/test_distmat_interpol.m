...
addpath(fullfile(mfilepath,'toolbox/'));
addpath(fullfile(mfilepath,'mexEMD/'));
addpath(fullfile(mfilepath,'mesh2d/'));
%addpath(fullfile(mfilepath,'toolbox_geometry/'));
%addpath(fullfile(mfilepath,'data/meshes/'));
%addpath(fullfile(mfilepath,'data/meshes-nonrigid/'));
addpath(fullfile(mfilepath,'data/shapes/'));
%addpath(fullfile(mfilepath,'data/shapes-nonrigid/'));
...
for k=1:length(tlist)
    ...
    [Kbary,R,a,b] = compute_gw_barycenters(K,K{init},lambda,epsilon,options);
    plot(sort(a))
    plot(sort(b))
    Dbary = iKembed(Kbary);
    DbaryList{k} = Dbary;
